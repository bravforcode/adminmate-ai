  ...
  if (!open) {
    return (
      <button
        ...
        className="flex items-center gap-2 px-3 py-[7px] rounded-lg border border-border bg-surface-sunken/50 text-ink-variant text-[13px] cursor-pointer transition-all duration-200 min-w-0 sm:min-w-[200px] max-w-[320px] flex-1 hover:bg-surface-sunken focus-visible:ring-2 focus-visible:ring-primary/30 focus-visible:outline-none"
      >
        <Search size={15} />
        <span className="flex-1 text-left">{t('search.placeholder')}</span>
        <kbd className="flex items-center gap-[2px] px-1.5 py-0.5 rounded text-[11px] font-inherit bg-surface border border-border text-ink-variant">
          <Command size={10} />K
        </kbd>
      </button>
    )
        ...
        >
          ...
          <div className="flex items-center gap-3 px-5 py-4 border-b border-border/50">
            ...
            <input
              ...
              className="flex-1 border-none outline-none bg-transparent text-[15px] text-ink placeholder:text-ink-variant/50"
            />
            <button
              onClick={close}
              aria-label={t('common.close')}
              className="p-2 rounded-md text-ink-variant hover:text-ink hover:bg-surface-sunken transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center"
            >
              <X size={16} />
            </button>
          </div>
          ...
          <div className="overflow-y-auto flex-1 py-2">
            ...
            {!query.trim() && (
              <div className="px-5 py-6 text-center">
                <p className="text-[13px] text-ink-variant m-0 mb-4">
                  {t('search.tip')}
                </p>
                <div className="flex justify-center gap-4 flex-wrap">
                  <span className="text-xs text-ink-variant flex items-center gap-1">
                ...
                {recentSearches.length > 0 && (
                  <div className="mt-5 pt-3 border-t border-border/50">
                    <p className="text-[11px] text-ink-variant m-0 mb-2 uppercase tracking-wider">
                      {t('search.recent')}
                    </p>
                    <div className="flex flex-wrap gap-1.5 justify-center">
                      {recentSearches.map(r => (
                        <button
                          ...
                          className="flex items-center gap-1 px-2.5 py-1 rounded-md border border-border bg-surface-sunken/50 text-ink-variant text-xs cursor-pointer hover:bg-surface-sunken transition-colors"
                        >
                          <Clock size={11} /> {r}
                        </button>
                      ))}
            ...
            {query.trim() && totalResults === 0 && !loading && (
              <div className="py-8 px-5 text-center">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-full bg-surface-sunken mb-3">
                  <Search size={20} className="text-ink-variant" />
                </div>
                <p className="text-sm text-ink-variant m-0">
                  {t('search.no_results')}
                </p>
                <p className="text-xs text-ink-variant/60 mt-1 m-0">
            ...
            {categoryResults.map(({ type, items }) => {
              ...
              return (
                <div key={type}>
                  <p className="px-5 pt-2 pb-1 text-[11px] font-semibold uppercase tracking-wider text-ink-variant m-0 transition-colors">
                    {t(meta.labelKey)}
                  </p>
                  {items.map(item => (
                    <button
                    ...
                    >
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] text-ink-variant m-0 truncate">
                          {item.subtitle}
                        </p>
                      </div>
                    </button>
          ...
          {/* Footer hint */}
          <div className="px-5 py-2.5 border-t border-border/50 flex items-center justify-between text-[11px] text-ink-variant">
