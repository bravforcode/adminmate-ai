...
export function CVParseResult({ cvDocument }: CVParseResultProps) {
  ...
  return (
    <div className="space-y-4" data-testid="cv-parsed-badge">
      {parsed.summary && <div><h4 className="text-sm font-semibold mb-1">{t('recruitment.candidates.cv_summary', 'Summary')}</h4><p className="text-sm text-on-surface-variant">{parsed.summary}</p></div>}
      ...
      {parsed.work_experience && parsed.work_experience.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold mb-2 flex items-center gap-2"><Calendar size={14} /> {t('recruitment.candidates.cv_experience', 'Experience')}</h4>
          {parsed.work_experience.map((exp: CVParsedWorkExperience, i: number) => (
            <div key={i} className="mb-3 pl-4 border-l-2 border-outline-variant">
              ...
              <p className="text-xs text-on-surface-variant">{exp.start_date} - {exp.end_date || t('recruitment.candidates.cv_present', 'Present')}</p>
      ...
      {parsed.education && parsed.education.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold mb-2 flex items-center gap-2"><Award size={14} /> {t('recruitment.candidates.cv_education', 'Education')}</h4>
      ...
      {parsed.languages && parsed.languages.length > 0 && (
        <div>
          <h4 className="text-sm font-semibold mb-2 flex items-center gap-2"><Languages size={14} /> {t('recruitment.candidates.cv_languages', 'Languages')}</h4>
