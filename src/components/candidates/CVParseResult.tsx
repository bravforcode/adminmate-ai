    ...
    <div className="space-y-4" data-testid="cv-parsed-badge">
      ...
      {parsed.languages && parsed.languages.length > 0 && (
        <div>
          ...
          <div className="flex gap-2">{parsed.languages.map((l: CVParsedLanguage, i: number) => <span key={i} className="px-2 py-0.5 bg-surface-sunken rounded text-xs">{l.name}: {l.level}</span>)}</div>
        </div>
      )}
    </div>
  )
