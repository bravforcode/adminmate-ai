    ...
    <div className="flex flex-col h-[calc(100vh-180px)]">
      <div className="flex-1 chat-scroll space-y-4 pr-2">
        {messages.length === 0 && (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-on-surface-variant mb-6">{t('empty.chat_welcome_description')}</p>
            <div className="flex flex-wrap justify-center gap-2">
              {suggestionItems.map(s => (
                <button key={s} onClick={() => { setInput(s); setTimeout(() => handleSend(), 100) }}
                  className="px-3 py-1.5 bg-surface-container-low dark:bg-surface-container rounded-full text-sm hover:bg-surface-container-high dark:hover:bg-outline transition-colors text-on-surface-variant">{s}</button>
              ))}
            </div>
          </div>
        )}
        {messages.map(msg => (
          <div key={msg.id} className={`flex gap-3 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
            ...
            <div className={`max-w-[75%] rounded-xl p-3 text-sm ${msg.sender === 'user' ? 'bg-primary text-on-primary' : 'bg-surface-container-low dark:bg-surface-container text-on-surface'}`}>
        ...
        {isLoading && (
          <div className="flex gap-3">
            ...
            <div className="bg-surface-container-low dark:bg-surface-container rounded-xl p-3 text-sm">
      ...
      <div className="flex gap-2 mt-4 pt-4 border-t border-outline-variant">
        <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()}
          className="flex-1 px-4 py-3 rounded-full border border-outline-variant bg-surface-container-lowest text-on-surface focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all duration-200 outline-none text-sm"
