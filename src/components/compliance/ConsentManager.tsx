...
export function ConsentManager() {
  ...
  if (isLoading) {
    return <div className="space-y-2">{[1, 2].map(i => <div key={i} className="h-16 bg-surface-container-high rounded-lg animate-shimmer" />)}</div>
  }

  return (
    <div className="space-y-3">
      {consents?.map(consent => (
        <div key={consent.id} className="border border-outline-variant rounded-lg overflow-hidden">
          <button
          ...
          {expandedId === consent.id && (
            <div className="px-3 pb-3 border-t border-outline-variant pt-2 space-y-2">
      ...
      {consents?.length === 0 && (
        <p className="text-sm text-on-surface-variant text-center py-4">{t('pdpa.no_consents') || 'No consent records found.'}</p>
      )}
    </div>
  )
}
