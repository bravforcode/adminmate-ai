...
export function InterviewFeedbackForm({ interview, onClose }: Props) {
  ...
  const recommendations = [
    ...
    { value: 'neutral', label: t('interviews.feedback.neutral'), icon: Minus, color: 'text-gray-500 dark:text-ink-variant' },
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <h3 className="font-semibold dark:text-ink">{t('interviews.feedback.title')}</h3>
      <div>
        <label className="block text-sm font-medium mb-2 dark:text-ink">{t('interviews.feedback.rating')}</label>
      ...
      <div>
        <label className="block text-sm font-medium mb-2 dark:text-ink">{t('interviews.feedback.recommendation')}</label>
      ...
      <div>
        <label htmlFor="interview-feedback" className="block text-sm font-medium mb-1 dark:text-ink">{t('interviews.feedback.detailed_feedback')}</label>
        <textarea id="interview-feedback" value={feedback} onChange={e => setFeedback(e.target.value)} rows={4} className="w-full px-4 py-3 rounded-xl border border-border bg-surface-sunken-lowest dark:bg-surface-sunken-lowest text-ink focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all duration-200 outline-none" placeholder={t('interviews.feedback.placeholder')} />
      </div>
