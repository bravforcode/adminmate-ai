  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div>
        ...
        <input {...register('email')} type="email" data-testid="email-input"
      ...
      <div>
        ...
        <div className="relative">
          <input {...register('password')} type={showPassword ? 'text' : 'password'} data-testid="password-input"
      ...
      <div className="relative my-4">
        ...
        <div className="relative flex justify-center text-sm"><span className="px-2 bg-surface text-on-surface-variant">{t('auth.or')}</span></div>
      </div>
      ...
      <button type="button" onClick={() => {
        ...
        className="w-full bg-green-600 text-white py-2.5 rounded-lg font-label-md hover:bg-green-700 transition-colors flex items-center justify-center gap-2 mt-2">
        <span>🔑</span> Demo Access (Bypass Login)
      </button>

      <div className="flex justify-between text-sm mt-4">
        <Link to="/forgot-password" className="text-primary hover:underline">{t('auth.forgot_password')}</Link>
        <Link to="/register" className="text-primary hover:underline">{t('auth.create_account')}</Link>
      </div>
    </form>
  )
}
