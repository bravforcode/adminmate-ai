...
export function useUpdateDocument() {
  const qc = useQueryClient()
  const company = useAuthStore(s => s.company)
  return useMutation({ mutationFn: ({ id, data }: { id: string; data: any }) => documentService.update(id, data), onSuccess: () => { qc.invalidateQueries({ queryKey: ['documents', company?.id] }) }, onError: (e: Error) => toast.error(e.message) })
}
