    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-surface rounded-xl p-5 border border-border shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-sm font-medium text-ink-variant dark:text-ink-variant">{t('stats.total_hours')}</span>
          </div>
          <p className="text-2xl font-bold text-on-background dark:text-ink">{stats.totalHours}h</p>
        </div>
        <div className="bg-surface rounded-xl p-5 border border-border shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-sm font-medium text-ink-variant dark:text-ink-variant">{t('stats.overtime')}</span>
          </div>
          <p className="text-2xl font-bold text-on-background dark:text-ink">{stats.overtime}h</p>
        </div>
        <div className="bg-surface rounded-xl p-5 border border-border shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-sm font-medium text-ink-variant dark:text-ink-variant">{t('stats.late_arrivals')}</span>
          </div>
          <p className="text-2xl font-bold text-on-background dark:text-ink">{stats.lateArrivals}</p>
        </div>
        <div className="bg-surface rounded-xl p-5 border border-border shadow-sm">
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className="text-sm font-medium text-ink-variant dark:text-ink-variant">{t('stats.present_days')}</span>
          </div>
          <p className="text-2xl font-bold text-on-background dark:text-ink">{stats.presentDays}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
        <input
          ...
          className="px-4 py-2 rounded-xl border border-border bg-surface-sunken-lowest dark:bg-surface-sunken-lowest text-ink focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all text-sm w-full sm:w-64"
        />
        <select
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as AttendanceStatus | '')}
          aria-label={t('filter_all_statuses')}
          className="px-4 py-2 rounded-xl border border-border bg-surface-sunken-lowest dark:bg-surface-sunken-lowest text-ink focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all text-sm w-full sm:w-48"
        >
        ...
        <CardContent className="p-0">
          {isLoading ? (
            ...
            <div className="grid grid-cols-7 border-b border-border overflow-x-auto">
              {WEEKDAYS.map(day => (
                <div key={day} className="py-2 px-1 text-center text-[10px] sm:text-xs font-semibold text-ink-variant dark:text-ink-variant border-r border-border/50 dark:border-border/50 last:border-r-0">
                  {t(`weekday.${day.toLowerCase()}`)}
                </div>
              ))}
              {Array.from({ length: startPadding }).map((_, i) => (
                <div key={`pad-${i}`} className="min-h-[60px] sm:min-h-[100px] border-r border-b border-border/50 dark:border-border/50 last:border-r-0 bg-surface-sunken-lowest/50 dark:bg-surface-sunken/20" />
              ))}
              {calendarDays.map(({ day, date }) => {
                ...
                return (
                  <div
                    key={day}
                    className={cn(
                      'min-h-[60px] sm:min-h-[100px] p-1 sm:p-1.5 border-r border-b border-border/50 dark:border-border/50 last:border-r-0 transition-colors hover:bg-surface-sunken/30 dark:hover:bg-surface-sunken/20',
            ...
            <tbody className="text-sm text-ink dark:text-ink">
              {records.length === 0 ? (
                ...
                records.slice(0, 10).map(r => (
                  <tr key={r.id} className="border-b border-border/50 dark:border-border/50 hover:bg-surface-sunken/50 dark:hover:bg-surface-sunken/30 transition-colors duration-150">
