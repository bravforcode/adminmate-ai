    ...
    <div className="space-y-3">
      {data.map((item, i) => (
        <div key={item.label} className="flex items-center gap-3">
          <span className="text-xs text-ink-muted w-24 truncate">{item.label}</span>
    ...
    <div className="flex items-end gap-2 h-40">
      {data.map((item, i) => (
        <div key={item.label} className="flex-1 flex flex-col items-center gap-1">
          ...
          <span className="text-[10px] text-ink-muted truncate w-full text-center">{item.label}</span>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-headline-md md:text-headline-lg font-bold text-on-background">{t('people_analytics.title', 'People Analytics')}</h1>
          <p className="text-body-md text-ink-muted mt-1">{t('people_analytics.subtitle', 'Workforce metrics and insights')}</p>
        </div>
                ...
                {Object.entries(metrics.deptTermMap).map(([dept, data]) => {
                  ...
                  return (
                    <div key={dept} className="p-4 rounded-lg border border-border bg-surface-sunken">
                      ...
                      <p className="text-xs text-ink-muted mt-1">{data.total} total · {data.terminated} left</p>
                    </div>
                  )
                })}
              </div>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="p-3 rounded-lg bg-surface-sunken text-center">
                    ...
                    <p className="text-xs text-ink-muted">{t('pa.avg_days_hire', 'Avg Days to Hire')}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-surface-sunken text-center">
                    <p className="text-2xl font-bold text-ink">{metrics.acceptanceRate}%</p>
                    <p className="text-xs text-ink-muted">{t('pa.offer_acceptance', 'Offer Acceptance')}</p>
                  </div>
                </div>
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between"><span className="text-ink-muted">{t('pa.total_applications', 'Total Applications')}</span><span className="font-semibold">{metrics.totalApps}</span></div>
                  <div className="flex justify-between"><span className="text-ink-muted">{t('pa.hired', 'Hired')}</span><span className="font-semibold text-green-600">{metrics.hiredCount}</span></div>
                  <div className="flex justify-between"><span className="text-ink-muted">{t('pa.rejected', 'Rejected')}</span><span className="font-semibold text-red-600">{metrics.rejectedCount}</span></div>
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <div className="grid grid-cols-2 gap-4 mb-4">
                  <div className="p-3 rounded-lg bg-surface-sunken text-center">
                    ...
                    <p className="text-xs text-ink-muted">{t('pa.avg_salary', 'Avg Salary')}</p>
                  </div>
                  <div className="p-3 rounded-lg bg-surface-sunken text-center">
                    <p className="text-2xl font-bold text-ink">${metrics.medianSalary.toLocaleString()}</p>
                    <p className="text-xs text-ink-muted">{t('pa.median_salary', 'Median Salary')}</p>
                  </div>
                </div>
                <p className="text-xs text-ink-muted mb-2">Based on {metrics.totalSalaries} employees</p>
              </CardContent>
            </Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="p-4 rounded-lg border border-border bg-surface-sunken text-center">
                  ...
                  <p className="text-xs text-ink-muted">{t('pa.absence_rate', 'Absence Rate')}</p>
                </div>
                <div className="p-4 rounded-lg border border-border bg-surface-sunken text-center">
                  <p className="text-2xl font-bold text-ink">{metrics.approvedLeaves}</p>
                  <p className="text-xs text-ink-muted">{t('pa.approved_leaves', 'Approved Leaves')}</p>
                </div>
                <div className="p-4 rounded-lg border border-border bg-surface-sunken text-center">
                  <p className="text-2xl font-bold text-orange-600">{metrics.sickLeaves}</p>
                  <p className="text-xs text-ink-muted">{t('pa.sick_leaves', 'Sick Leaves')}</p>
                </div>
              </div>
            </CardContent>
          </Card>
