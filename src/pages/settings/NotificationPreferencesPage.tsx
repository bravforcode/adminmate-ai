...
export function NotificationPreferencesPage() {
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="space-y-4 skeleton-stagger">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-16 bg-surface-container-high rounded-xl animate-shimmer" />
          ))}
        </div>
      </div>
    )
  ...
  return (
    <div className="space-y-6">
      <div>
        <Link to="/settings" className="inline-flex items-center gap-1 text-sm text-on-surface-variant hover:text-primary mb-3">
          <ArrowLeft size={14} /> {t('settings.title') || 'Settings'}
        </Link>
        <h1 className="text-headline-md font-bold text-on-surface">{t('notifications_prefs.title') || 'Notification Preferences'}</h1>
        <p className="text-body-md text-on-surface-variant mt-1">{t('notifications_prefs.subtitle') || 'Choose how you want to be notified'}</p>
      </div>
      ...
      <div className="bg-surface rounded-xl border border-outline-variant shadow-sm p-6">
        <div className="flex items-center gap-2 mb-6 border-b border-surface-container pb-3">
          ...
          <h3 className="text-title-lg font-semibold text-on-surface">{t('notifications_prefs.channels') || 'Notification Channels'}</h3>
        </div>

        <div className="grid grid-cols-12 text-xs font-medium text-on-surface-variant mb-3 px-1">
            ...
            return (
              <div key={type} className="grid grid-cols-12 items-center py-4 px-1 gap-2">
                <div className="col-span-5 sm:col-span-4">
                  <div className="text-sm font-medium text-on-surface">{t(labelKey) || type.replace(/_/g, ' ')}</div>
                  <div className="text-xs text-on-surface-variant mt-0.5">{t(descKey) || ''}</div>
                </div>
                <div className="col-span-1 flex justify-center">
                  <Smartphone size={14} className="text-on-surface-variant hidden sm:block" />
                </div>
                <div className="col-span-3 sm:col-span-2 flex justify-center">
                  <div className="flex items-center gap-2">
                    <Mail size={14} className="text-on-surface-variant hidden sm:block" />
                    <Toggle
