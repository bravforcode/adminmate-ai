  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-headline-md md:text-headline-lg font-bold text-on-background dark:text-on-surface">{t('export_data.title')}</h1>
          <p className="text-body-md text-on-surface-variant dark:text-on-surface-variant mt-1">{t('export_data.subtitle')}</p>
        </div>
      </header>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-2 text-on-surface dark:text-on-surface">{t('export_data.entity_type')}</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1 text-on-surface dark:text-on-surface">{t('export_data.date_from')}</label>
              <input
                type="date"
                value={dateFrom}
                onChange={(e) => setDateFrom(e.target.value)}
                className="w-full px-3 py-2 rounded-xl border border-outline-variant dark:border-outline bg-surface-container-lowest dark:bg-surface-container-lowest text-on-surface text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1 text-on-surface dark:text-on-surface">{t('export_data.date_to')}</label>
              <input
                type="date"
                value={dateTo}
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-on-surface dark:text-on-surface">{t('export_data.status')}</label>
              <select
                value={statusFilter}
          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="border-b border-outline-variant/50 dark:border-outline/50">
                <th className="px-4 py-3 text-left font-medium text-on-surface-variant">{t('export_data.table.entity')}</th>
                <th className="px-4 py-3 text-left font-medium text-on-surface-variant">{t('export_data.table.status')}</th>
                <th className="px-4 py-3 text-left font-medium text-on-surface-variant">{t('export_data.table.date')}</th>
                <th className="px-4 py-3 text-left font-medium text-on-surface-variant">{t('export_data.table.download')}</th>
              </tr>
            </thead>
            <tbody>
              {historyLoading ? (
                ...
                <tr><td colSpan={4} className="py-8 text-center text-on-surface-variant">{t('export_data.no_history')}</td></tr>
              ) : (
