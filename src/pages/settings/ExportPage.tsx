  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-headline-md md:text-headline-lg font-bold text-on-background dark:text-ink">{t('export_data.title')}</h1>
          <p className="text-body-md text-ink-variant dark:text-ink-variant mt-1">{t('export_data.subtitle')}</p>
        </div>
      </header>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-2 text-ink dark:text-ink">{t('export_data.entity_type')}</label>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1 text-ink dark:text-ink">{t('export_data.date_from')}</label>
              <input
                type="date"
                value={dateFrom}
                onChange={(e) => setDateFrom(e.target.value)}
                className="w-full px-3 py-2 rounded-xl border border-border bg-surface-sunken-lowest dark:bg-surface-sunken-lowest text-ink text-sm focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all"
              />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1 text-ink dark:text-ink">{t('export_data.date_to')}</label>
              <input
                type="date"
                value={dateTo}
            ...
            <div>
              <label className="block text-sm font-medium mb-1 text-ink dark:text-ink">{t('export_data.status')}</label>
              <select
                value={statusFilter}
