    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface rounded-xl border border-outline-variant p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-title-lg font-semibold text-on-surface">{t('compliance.score_title') || 'Compliance Score'}</h3>
              <p className="text-sm text-on-surface-variant">{t('compliance.score_subtitle', { country }) || `${country} regulatory framework`}</p>
            </div>
          </div>
          <div className="text-right">
            ...
            <p className="text-xs text-on-surface-variant">{passedChecks}/{checks.length} {t('compliance.checks_passed') || 'checks passed'}</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="bg-surface rounded-xl border border-outline-variant p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-on-surface-variant">{t('compliance.active_consents') || 'Active Consents'}</p>
                <p className="text-xl font-bold text-on-surface">{consentStats?.active ?? 0}</p>
              </div>
            </div>
            <div className="bg-surface rounded-xl border border-outline-variant p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-on-surface-variant">{t('compliance.pending_requests') || 'Pending Requests'}</p>
                <p className="text-xl font-bold text-on-surface">{deletionRequests?.filter(r => r.status === 'pending').length ?? 0}</p>
              </div>
            </div>
            <div className="bg-surface rounded-xl border border-outline-variant p-4 flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-on-surface-variant">{t('compliance.retention_policies') || 'Retention Policies'}</p>
                <p className="text-xl font-bold text-on-surface">{retentionPolicies?.length ?? 0}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface rounded-xl border border-outline-variant p-6">
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div className="bg-surface-container-low rounded-lg p-4 text-center">
              <p className="text-2xl font-bold text-on-surface">{consentStats?.total ?? 0}</p>
              <p className="text-xs text-on-surface-variant">{t('compliance.total_consents') || 'Total Consents'}</p>
            </div>
            <div className="bg-surface-container-low rounded-lg p-4 text-center">
          ...
          {retentionPolicies && retentionPolicies.length > 0 ? (
            <div className="space-y-2">
              {retentionPolicies.map((policy: Record<string, unknown>) => (
                <div key={policy.id as string} className="flex items-center justify-between py-3 px-3 rounded-lg bg-surface-container-low border border-outline-variant">
                  <div>
                    <p className="text-sm font-medium text-on-surface">{policy.entity_type as string}</p>
                    <p className="text-xs text-on-surface-variant">{t('compliance.retention_action') || 'Action'}: {policy.action as string}</p>
                  </div>
      ...
      {activeTab === 'requests' && (
        <div className="bg-surface rounded-xl border border-outline-variant p-6">
          ...
          {requestsLoading ? (
            <div className="space-y-2">{[1, 2, 3].map(i => <div key={i} className="h-16 bg-surface-container-high rounded-lg animate-shimmer" />)}</div>
          ...
          {recentAuditLogs && recentAuditLogs.length > 0 ? (
            <div className="space-y-2">
              {recentAuditLogs.map((log: AuditLogEntry) => (
                <div key={log.id} className="flex items-center justify-between py-2 px-3 rounded-lg bg-surface-container-low">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-on-surface-variant">{log.user_profiles?.full_name || 'System'}</span>
                  </div>
                  <span className="text-xs text-on-surface-variant">{new Date(log.created_at).toLocaleString()}</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-on-surface-variant">{t('empty.audit_log_title')}</p>
          )}
        </div>
      )}
    </div>
