      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative max-w-sm flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-3 rounded-xl border border-outline-variant bg-surface-container-lowest text-on-surface focus:border-primary focus:ring-2 focus:ring-primary/20 transition-all outline-none text-sm placeholder:text-on-surface-variant/50"
        ...
        {sources.length > 0 && (
          <div className="relative">
            ...
            <select
              ...
              className="pl-9 pr-8 py-3 rounded-xl border border-outline-variant bg-surface-container-lowest text-on-surface text-sm outline-none appearance-none cursor-pointer"
            >
        ...
        <div className="flex gap-1 bg-surface-container-low rounded-lg p-1">
          <button
            ...
            className={`p-2 rounded-md transition-colors ${viewMode === 'grid' ? 'bg-surface shadow-sm text-on-surface' : 'text-on-surface-variant hover:text-on-surface'}`}
          ...
          <button
            ...
            className={`p-2 rounded-md transition-colors ${viewMode === 'list' ? 'bg-surface shadow-sm text-on-surface' : 'text-on-surface-variant hover:text-on-surface'}`}
          ...
          {viewMode === 'grid' ? (
              ...
              {paginated.map(c => (
                <div key={c.id} className="relative group">
                  <button
                    ...
                    className={`absolute top-2 left-2 z-10 p-1 rounded ${selectedIds.has(c.id) ? 'text-primary' : 'text-on-surface-variant opacity-0 group-hover:opacity-100'} transition-opacity`}
                  >
            ...
            <div className="border border-outline-variant rounded-xl overflow-hidden">
              <table className="w-full text-sm">
                  ...
                  <tr>
                    <th className="p-3 text-left w-10">
                      <button onClick={handleSelectAll}>
                        {selectedIds.size === paginated.length ? <CheckSquare size={16} className="text-primary" /> : <Square size={16} className="text-on-surface-variant" />}
                      </button>
                    </th>
                ...
                <tbody>
                  {paginated.map(c => (
                    <tr key={c.id} className="border-t border-outline-variant hover:bg-surface-container-low transition-colors">
                      <td className="p-3">
                        <button onClick={() => handleToggleSelect(c.id)}>
                          {selectedIds.has(c.id) ? <CheckSquare size={16} className="text-primary" /> : <Square size={16} className="text-on-surface-variant" />}
                        </button>
                      </td>
                      <td className="p-3">
                        <Link to={`/recruitment/candidates/${c.id}`} className="font-medium text-on-surface hover:text-primary hover:underline">
                          {c.full_name}
                        </Link>
                      </td>
                      <td className="p-3 text-on-surface-variant hidden md:table-cell">{c.email ?? '—'}</td>
                      <td className="p-3 text-on-surface-variant hidden lg:table-cell">{c.current_position ?? '—'}</td>
                      <td className="p-3 text-on-surface-variant hidden sm:table-cell">{c.source ?? '—'}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
