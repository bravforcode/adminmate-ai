...
import { Check, Plus, Send, BookOpen, Heart, Scale, ExternalLink, Bot, AlertCircle, RefreshCw, Users } from 'lucide-react'
...
export function OnboardingMgmtPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-headline-md font-bold text-on-surface">{t('onboarding.hub')}</h1>
        <p className="text-body-md text-on-surface-variant mt-1">{t('onboarding.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 space-y-6 w-full min-w-0">
          <div className="bg-surface rounded-xl border border-outline-variant p-6 shadow-sm">
            <h3 className="text-title-lg font-semibold text-on-surface mb-4">{t('onboarding.my_checklist')}</h3>
            {myChecklist ? (
              <>
                <div className="mb-4">
              ...
              <p className="text-sm text-on-surface-variant py-4">{t('onboarding.no_checklist')}</p>
            )}
          </div>

          <div className="bg-surface rounded-xl border border-outline-variant p-6 shadow-sm">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-4">
              <h3 className="text-title-lg font-semibold text-on-surface">{t('onboarding.team_status')}</h3>
              <button onClick={() => navigate('/recruitment/candidates')} className="text-sm text-primary hover:underline self-start sm:self-auto">{t('onboarding.view_all')}</button>
            </div>
            {isError ? (
              <div className="bg-surface rounded-xl border border-error/30 p-6 text-center">
                <AlertCircle size={32} className="mx-auto text-error mb-2" />
                <h4 className="font-semibold text-on-surface mb-1">{t('onboarding.error_title')}</h4>
                <p className="text-sm text-on-surface-variant mb-3">{(error as Error)?.message || t('onboarding.error_description')}</p>
                <button
                  onClick={() => refetch()}
                  ...
                  <table className="w-full text-left border-collapse min-w-[500px]">
                    <thead>
                      <tr className="border-b border-outline-variant">
                        <th className="py-2 px-3 text-xs font-semibold text-on-surface-variant">{t('onboarding.employee')}</th>
                        <th className="py-2 px-3 text-xs font-semibold text-on-surface-variant">{t('onboarding.role')}</th>
                        <th className="py-2 px-3 text-xs font-semibold text-on-surface-variant">{t('onboarding.progress')}</th>
                        <th className="py-2 px-3 text-xs font-semibold text-on-surface-variant">{t('onboarding.status')}</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr key={cl.id} className="border-b border-surface-container last:border-0">
                          ...
                          <td className="py-3 px-3">
                            <span className={cn(
                              ...
                              {cl.status === 'completed' ? t('onboarding.completed') : cl.progress_percentage < 20 ? t('onboarding.at_risk') : t('onboarding.on_track')}
                            </span>
                          </td>
                        </tr>
                      ))}
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="bg-surface rounded-xl border border-outline-variant shadow-sm flex flex-col h-[400px] overflow-hidden">
            <div className="bg-primary-container p-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-inverse-primary">{t('onboarding.ai.hr_assistant')}</p>
              </div>
            </div>
          ...
          <div className="bg-surface rounded-xl border border-outline-variant p-6 shadow-sm">
            <h3 className="text-title-lg font-semibold text-on-surface mb-4">{t('onboarding.quick_resources')}</h3>
            ...
            {acceptedOffers && acceptedOffers.length > 0 ? (
              acceptedOffers.map((offer: Offer) => (
                <div key={offer.id} className="flex items-center justify-between py-2 border-b border-outline-variant last:border-0">
                  ...
                  >
                    <Plus size={12} /> {t('onboarding.create_checklist')}
                  </button>
                </div>
              ))
            ) : (
              <p className="text-sm text-on-surface-variant py-4">{t('onboarding.no_offers')}</p>
            )}
          </div>
        </div>
      </div>
