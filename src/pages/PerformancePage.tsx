...
const STATUS_COLORS: Record<CycleStatus, string> = {
  draft: 'bg-gray-100 text-gray-700 dark:text-ink-variant',
  ...
  return (
    <div className="bg-surface rounded-xl p-5 border border-border shadow-sm">
      <div className="flex items-center gap-3 mb-3">
        ...
        <span className="text-sm text-ink-variant dark:text-ink-variant">{title}</span>
      </div>
      <p className="text-2xl font-bold text-ink dark:text-ink">{value}</p>
    </div>
  )
}
...
export function PerformancePage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-headline-md font-bold text-ink dark:text-ink">{t('title', 'Performance Management')}</h1>
        <p className="text-body-md text-ink-variant dark:text-ink-variant mt-1">{t('subtitle', 'Manage review cycles, track performance, and identify talent')}</p>
      </div>
      ...
      <div className="flex gap-1 border-b border-border dark:border-border" role="tablist" aria-label={t('performance.tabs_label', 'Performance sections')}>
                ...
                <table role="table" className="table-card-mobile w-full text-left min-w-[600px]">
                  <thead>
                    <tr className="bg-surface-sunken dark:bg-surface-sunken/50 border-b border-border/50 dark:border-border/50">
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('cycle_name', 'Cycle Name')}</th>
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('type', 'Type')}</th>
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('period', 'Period')}</th>
                      ...
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('reviews', 'Reviews')}</th>
                  ...
                  <tbody>
                    {filteredCycles.map(cycle => {
                      ...
                      return (
                        <tr key={cycle.id} className="hover:bg-surface-sunken/50 dark:hover:bg-surface-sunken/30 transition-colors duration-150 border-b border-border/50 dark:border-border/50">
                          <td className="py-3 px-4 text-sm text-ink dark:text-ink" data-label={t('cycle_name', 'Cycle Name')}>
                            <span className="font-medium">{cycle.name}</span>
                          </td>
                          <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant" data-label={t('type', 'Type')}>{cycle.cycle_type}</td>
                          <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant" data-label={t('period', 'Period')}>
                          ...
                          <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant" data-label={t('reviews', 'Reviews')}>
            ...
            <div className="bg-surface rounded-xl border border-border overflow-hidden">
              <div className="table-scroll">
                <table role="table" className="table-card-mobile w-full text-left min-w-[600px]">
                  <thead>
                    <tr className="bg-surface-sunken dark:bg-surface-sunken/50 border-b border-border/50 dark:border-border/50">
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('employee', 'Employee')}</th>
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('review_type', 'Review Type')}</th>
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('status', { ns: 'common' })}</th>
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('rating', 'Rating')}</th>
                      <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('submitted', 'Submitted')}</th>
                    </tr>
                  </thead>
                  <tbody>
                    {reviews.map(review => (
                      <tr key={review.id} className="hover:bg-surface-sunken/50 dark:hover:bg-surface-sunken/30 transition-colors duration-150 border-b border-border/50 dark:border-border/50">
                        <td className="py-3 px-4 text-sm font-medium text-ink dark:text-ink" data-label={t('employee', 'Employee')}>
                          {review.user_profiles?.full_name || '-'}
                        </td>
                        <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant" data-label={t('review_type', 'Review Type')}>
                          {review.review_type}
                        </td>
                        <td className="py-3 px-4 text-sm" data-label={t('status', { ns: 'common' })}>
                        ...
                        <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant" data-label={t('rating', 'Rating')}>
                          {review.overall_rating != null ? `${review.overall_rating}/5` : '-'}
                        </td>
                        <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant" data-label={t('submitted', 'Submitted')}>
            ...
            <>
              <div className="bg-surface rounded-xl border border-border p-6">
                <div className="flex items-center gap-2 mb-4">
                  <LayoutGrid size={18} className="text-ink-variant dark:text-ink-variant" />
                  <h3 className="text-title-sm font-semibold text-ink dark:text-ink">{t('nine_box_title', '9-Box Grid: Performance vs Potential')}</h3>
                </div>
                <p className="text-sm text-ink-variant dark:text-ink-variant mb-6">{t('nine_box_desc', 'Employees positioned by performance score (horizontal) and potential score (vertical)')}</p>

                <div className="grid grid-cols-3 gap-2 max-w-2xl">
                  {[7, 8, 9, 4, 5, 6, 1, 2, 3].map(pos => (
                    <div key={pos} className={`rounded-lg p-3 min-h-[100px] ${NINE_BOX_COLORS[pos] || 'bg-gray-50 dark:bg-surface-sunken'}`}>
                ...
                <div className="flex items-center gap-4 mt-4 text-xs text-ink-variant dark:text-ink-variant">
                  ...
                  <div className="table-scroll">
                    <table role="table" className="table-card-mobile w-full text-left min-w-[500px]">
                      <thead>
                        <tr className="bg-surface-sunken dark:bg-surface-sunken/50 border-b border-border/50 dark:border-border/50">
                          <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('employee', 'Employee')}</th>
                          <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('performance_score', 'Performance')}</th>
                          <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('potential_score', 'Potential')}</th>
                          <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('position', 'Position')}</th>
                          <th className="py-3 px-4 text-xs font-semibold uppercase tracking-wider text-ink-variant dark:text-ink-variant">{t('notes', 'Notes')}</th>
                        </tr>
                      </thead>
                      <tbody>
                        {assessments.map(a => (
                          <tr key={a.id} className="hover:bg-surface-sunken/50 dark:hover:bg-surface-sunken/30 transition-colors duration-150 border-b border-border/50 dark:border-border/50">
                            <td className="py-3 px-4 text-sm font-medium text-ink dark:text-ink">{a.user_profiles?.full_name || '-'}</td>
                            <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant">{a.performance_score ?? '-'}</td>
                            <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant">{a.potential_score ?? '-'}</td>
                            ...
                            <td className="py-3 px-4 text-sm text-ink-variant dark:text-ink-variant max-w-[200px] truncate">{a.notes || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
