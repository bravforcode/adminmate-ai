...
export default function DocumentSigningPage() {
  ...
  if (isLoading || state === 'loading') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-surface dark:bg-surface-sunken-lowest">
        <div className="flex flex-col items-center gap-3">
          <Loader2 size={32} className="animate-spin text-primary" />
          <p className="text-sm text-ink-variant dark:text-ink-variant">{t('common:loading')}</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-surface dark:bg-surface-sunken-lowest py-8 px-4">
      <div className="max-w-2xl mx-auto">
        {state === 'sign' && (
          <div className="bg-surface rounded-2xl border border-border p-6 sm:p-8 shadow-lg">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                <h1 className="text-lg font-semibold text-ink dark:text-ink">{t('sign_title')}</h1>
                <p className="text-sm text-ink-variant dark:text-ink-variant">{documentName}</p>
              </div>
            </div>

            <div className="mb-6 p-4 bg-surface-sunken dark:bg-surface-sunken/30 rounded-xl">
              <p className="text-sm text-ink-variant dark:text-ink-variant">{t('signing_as')}</p>
              <p className="font-medium text-ink dark:text-ink">{signerName}</p>
            </div>
            ...
            {!showDecline ? (
              <>
                ...
                <button
                  ...
                  className="w-full text-center text-sm text-ink-variant dark:text-ink-variant hover:text-error dark:hover:text-error transition-colors py-2"
                >
                  {t('decline_to_sign')}
                </button>
              </>
              ...
              <div className="space-y-4">
                ...
                <textarea
                  ...
                  className="w-full px-4 py-3 rounded-xl border border-border bg-surface-sunken-lowest dark:bg-surface-sunken-lowest text-sm focus:border-error dark:focus:border-error outline-none resize-none"
                  rows={3}
        ...
        {state === 'signed' && (
          <div className="bg-surface rounded-2xl border border-border p-8 shadow-lg text-center">
            ...
            <p className="text-ink-variant dark:text-ink-variant">{t('signed_message')}</p>
          </div>
        )}
        ...
        {state === 'declined' && (
          <div className="bg-surface rounded-2xl border border-border p-8 shadow-lg text-center">
            ...
            <p className="text-ink-variant dark:text-ink-variant">{t('declined_message')}</p>
          </div>
        )}
        ...
        {(state === 'error' || state === 'not_found') && (
          <div className="bg-surface rounded-2xl border border-border p-8 shadow-lg text-center">
            ...
            <p className="text-ink-variant dark:text-ink-variant">{t('invalid_link_message')}</p>
          </div>
        )}
      </div>
    </div>
