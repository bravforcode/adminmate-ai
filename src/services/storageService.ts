...
export const storageService = {
  uploadCV: async (candidateId: string, file: File) => {
    ...
    const { data, error } = await supabase.storage.from('cv-uploads').upload(filePath, file, { upsert: true })
  ...
  uploadLogo: async (companyId: string, file: File) => {
    ...
    const { data, error } = await supabase.storage.from('company-logos').upload(filePath, file, { upsert: true })
    if (error) throw error
