...
beforeAll(async () => {
  ...
  if (!(await isSupabaseReachable())) {
    console.warn('[26A.5.1] Supabase not reachable at ' + SUPABASE_URL + ' — skipping integration setup')
    return
  }
  ...
  if (!allTokensOk) {
    console.warn('[26A.5.1] Failed to obtain auth tokens for all test users — skipping')
    return
  }
  setupOk = true
})
...
describe('26A.5.1 — chat_messages cross-tenant UPDATE/DELETE', () => {
  ...
  it('A1: Company A PATCH blocked from Company B rows', async () => {
    if (!setupOk) return console.warn('[26A.5.1] SKIP — Supabase setup incomplete')
