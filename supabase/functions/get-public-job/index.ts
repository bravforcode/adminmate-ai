...
serve(async (req) => {
  ...
  try {
    ...
    if (!token || token.length > 32) {
      return new Response(JSON.stringify({ success: false, error: 'Invalid token' }), { status: 400, headers: h })
    }
    ...
    if (error) {
      ...
      return new Response(JSON.stringify({ success: false, error: 'Failed to fetch job' }), { status: 500, headers: h })
    }

    if (!data || data.length === 0) {
      return new Response(JSON.stringify({ success: false, error: 'Job not found or not published' }), { status: 404, headers: h })
    }

    return new Response(JSON.stringify({ success: true, job: data[0] }), { status: 200, headers: h })
  } catch (err) {
