...
serve(async (req) => {
  ...
  try {
    ...
    if (!templateKey || !ALLOWED_TEMPLATES.has(templateKey)) {
      return new Response(JSON.stringify({ success: false, error: 'Invalid templateKey' }), { status: 400, headers: JSON_HEADERS })
    }
    if (!to || typeof to !== 'string' || !EMAIL_RE.test(to) || to.length > 254) {
      return new Response(JSON.stringify({ success: false, error: 'Valid recipient email is required' }), { status: 400, headers: JSON_HEADERS })
    }
    if (!data || typeof data !== 'object') {
      return new Response(JSON.stringify({ success: false, error: 'data object is required' }), { status: 400, headers: JSON_HEADERS })
    }
    if (!['th', 'en', 'vi', 'id'].includes(language)) {
      return new Response(JSON.stringify({ success: false, error: 'Invalid language' }), { status: 400, headers: JSON_HEADERS })
    }

    const resendKey = Deno.env.get('RESEND_API_KEY')
    if (!resendKey) {
      return new Response(JSON.stringify({ success: false, error: 'Email service not configured' }), { status: 503, headers: JSON_HEADERS })
    }
    ...
    if (!result.ok) {
      ...
      return new Response(JSON.stringify({ success: false, error: 'Email provider rejected the request' }), { status: 502, headers: JSON_HEADERS })
    }

    const resendResult = await result.json().catch(() => ({}))
    logRequest({ function: FN, userId, durationMs: Date.now() - start, status: 200 })
    return new Response(JSON.stringify({ success: true, id: resendResult.id }), { headers: JSON_HEADERS })
  } catch (error: any) {
