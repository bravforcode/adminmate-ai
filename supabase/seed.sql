...
--
-- FK fix (20240620000066): user_profiles.id REFERENCES auth.users(id)
-- On a clean db reset, auth.users is empty (Supabase auth is separate).
-- This seed wraps auth-dependent data in a conditional DO block that
-- checks if the test auth user exists before inserting.

SET search_path = public;

-- ================================================================
-- SECTION 1: REFERENCE DATA (no auth dependency, always safe)
-- ================================================================
...
-- ============== CANDIDATES (20) ==============
-- Only depends on company_id, no auth dependency
...
-- ================================================================
-- SECTION 2: AUTH-DEPENDENT DATA (requires auth.users row)
-- ================================================================
-- On a clean db reset, auth.users is empty because Supabase auth
-- is a separate system. We check for the test user before inserting.
DO $$
DECLARE
  v_test_user_id UUID := 'a07ac229-65da-488a-a2e5-0aeec1474510';
BEGIN
  -- Check if test auth user exists; if not, skip all auth-dependent seed data
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE id = v_test_user_id) THEN
    RAISE NOTICE 'AUTH SEED SKIP: test auth user (%) not found in auth.users. Auth-dependent seed data skipped. Sign up via the app first.', v_test_user_id;
    RETURN;
  END IF;

  RAISE NOTICE 'AUTH SEED OK: test auth user (%) found. Seeding auth-dependent data.', v_test_user_id;
  ...
  ('22222222-0000-0000-0000-000000000001', '11111111-1111-1111-1111-111111111111', v_test_user_id, 'Senior Frontend Developer', 'นักพัฒนา Frontend อาวุโส', 'Engineering', 'Bangkok', 'full_time', 'senior', 'active', 2, 0, 80000, 140000, 'THB', ARRAY['React','TypeScript','Tailwind','Next.js'], 'Build modern HR web apps using React and TypeScript.', 'พัฒนาเว็บแอปพลิเคชัน HR สมัยใหม่ด้วย React และ TypeScript', ARRAY['Design and implement UI components','Mentor junior developers','Review pull requests'], ARRAY['5+ years React experience','Strong TypeScript','English communication'], '2026-12-31'),
  ('22222222-0000-0000-0000-000000000002', '11111111-1111-1111-1111-111111111111', v_test_user_id, 'Backend Engineer (Node.js)', 'วิศวกร Backend (Node.js)', 'Engineering', 'Bangkok', 'full_time', 'mid', 'active', 1, 0, 60000, 100000, 'THB', ARRAY['Node.js','PostgreSQL','Supabase','REST'], 'Build scalable APIs and data pipelines.', 'สร้าง API ที่ปรับขนาดได้และ data pipeline', ARRAY['Design REST APIs','Optimize queries','Write tests'], ARRAY['3+ years Node.js','Strong SQL','Supabase experience'], '2026-09-30'),
  ('22222222-0000-0000-0000-000000000003', '11111111-1111-1111-1111-111111111111', v_test_user_id, 'HR Business Partner', 'พันธมิตรธุรกิจ HR', 'People', 'Bangkok', 'full_time', 'senior', 'active', 1, 0, 70000, 110000, 'THB', ARRAY['HR','Recruitment','Onboarding','Labor Law'], 'Partner with engineering to scale the team.', 'ทำงานร่วมกับทีมวิศวกรรมเพื่อขยายทีม', ARRAY['Run recruitment cycle','Build onboarding','Advise on labor law'], ARRAY['5+ years HRBP','Strong English','PDPA knowledge'], '2026-08-31'),
  ('22222222-0000-0000-0000-000000000004', '11111111-1111-1111-1111-111111111111', v_test_user_id, 'Product Designer', 'นักออกแบบผลิตภัณฑ์', 'Design', 'Bangkok', 'full_time', 'mid', 'draft', 1, 0, 50000, 90000, 'THB', ARRAY['Figma','UI/UX','Design Systems'], 'Design beautiful, accessible interfaces.', 'ออกแบบ UI ที่สวยงามและเข้าถึงได้', ARRAY['Create wireframes','Maintain design system','Usability tests'], ARRAY['3+ years product design','Strong Figma','Portfolio'], NULL),
  ('22222222-0000-0000-0000-000000000005', '11111111-1111-1111-1111-111111111111', v_test_user_id, 'DevOps Engineer', 'วิศวกร DevOps', 'Engineering', 'Remote', 'full_time', 'senior', 'closed', 1, 1, 90000, 150000, 'THB', ARRAY['Kubernetes','AWS','Terraform','CI/CD'], 'Closed role (already filled).', 'ปิดรับสมัครแล้ว (เติมตำแหน่งแล้ว)', ARRAY['Manage infrastructure','Automate deployments'], ARRAY['5+ years DevOps','AWS certified','Terraform experience'], '2025-06-30')
  ON CONFLICT (id) DO NOTHING;
  ...
  ('77777777-0000-0000-0000-000000000001','11111111-1111-1111-1111-111111111111',v_test_user_id,'66666666-0000-0000-0000-000000000001','TH_standard_v1','in_progress',40.0,'2026-06-15','2026-06-30',NULL),
  ('77777777-0000-0000-0000-000000000002','11111111-1111-1111-1111-111111111111',v_test_user_id,'66666666-0000-0000-0000-000000000002','TH_standard_v1','in_progress',20.0,'2026-07-01','2026-07-15',NULL),
  ('77777777-0000-0000-0000-000000000003','11111111-1111-1111-1111-111111111111',v_test_user_id,NULL,'TH_standard_v1','in_progress',10.0,'2026-06-01','2026-06-15',NULL),
  ('77777777-0000-0000-0000-000000000004','11111111-1111-1111-1111-111111111111',v_test_user_id,NULL,'TH_standard_v1','in_progress',60.0,'2026-05-15','2026-05-30',NULL),
  ('77777777-0000-0000-0000-000000000005','11111111-1111-1111-1111-111111111111',v_test_user_id,NULL,'TH_standard_v1','completed',100.0,'2026-04-01','2026-04-15','2026-04-14 17:00:00+07'),
  ('77777777-0000-0000-0000-000000000006','11111111-1111-1111-1111-111111111111',v_test_user_id,NULL,'TH_standard_v1','in_progress',0.0,'2026-07-15','2026-07-30',NULL),
  ('77777777-0000-0000-0000-000000000007','11111111-1111-1111-1111-111111111111',v_test_user_id,NULL,'TH_standard_v1','in_progress',80.0,'2026-05-01','2026-05-15',NULL),
  ('77777777-0000-0000-0000-000000000008','11111111-1111-1111-1111-111111111111',v_test_user_id,NULL,'TH_standard_v1','in_progress',30.0,'2026-06-20','2026-07-05',NULL),
  ('77777777-0000-0000-0000-000000000009','11111111-1111-1111-1111-111111111111',v_test_user_id,NULL,'TH_standard_v1','in_progress',15.0,'2026-07-01','2026-07-15',NULL),
  ('77777777-0000-0000-0000-000000000010','11111111-1111-1111-1111-111111111111',v_test_user_id,NULL,'TH_standard_v1','completed',100.0,'2026-03-01','2026-03-15','2026-03-14 16:00:00+07')
  ON CONFLICT (id) DO NOTHING;
